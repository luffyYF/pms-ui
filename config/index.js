...
module.exports = {
  dev: {
    ...
    proxyTable: {
      '/pms': {
        // target: "http://192.168.5.6/pms-back",//设置你调用的接口域名和端口号 别忘了加http
        target: "http://192.168.5.10:8084/",//设置你调用的接口域名和端口号 别忘了加http
        // target: "http://www.housefeel.cn/test-pms",
        secure: false,      // 如果是https接口，需要配置这个参数
        changeOrigin: true,  // 跨域带cookies
        pathRewrite: {
          '^/pms': ''//这里理解成用‘/api'代替target里面的地址，后面组件中我们掉接口时直接用api代替
      ...
      '/fs': {
        ...
        changeOrigin: true,    // 跨域带cookies
        pathRewrite: {
          '^/fs': ''//这里理解成用‘/api'代替target里面的地址，后面组件中我们掉接口时直接用api代替
          //比如我要调用'http://40.00.100.133:3002/user/login'，直接写‘/api/user/login'即可
        }
      }
    },
