// 特殊房设置
<template>
  <el-dialog title="特殊房设置" :visible.sync="dialogVisible" width="800px" :append-to-body="true" :close-on-click-modal="false" :before-close="handleClose">
    <el-form ref="form" :model="form" size="mini">
      <!-- input -->
      <div class="bg-reserve">
        <h5 class="info-title">特殊房基本参数设置</h5>
        <div class="info-li">
          <el-form-item>
          ...
          <el-form-item>
            特殊房提前&nbsp;&nbsp;<el-input v-model="form.remindTimeNum" size="mini" clearable></el-input>&nbsp;&nbsp;分钟提醒操作员
          </el-form-item>
          <!-- <el-form-item>
    ...
    <!-- table -->
      <div class="bg-reserve">
        <h5 class="info-title">特殊房房价方案参数设置</h5>
        <!-- <el-button type="primary" size="mini" style="margin-left:20px;" @click="addRow(tableData)">添加特殊房房价方案</el-button> -->
        <div class="info-li">
          <el-table
            size="mini"
            :data="tableData"
...
import {
  specialRoomRuleFind,
  specialRoomRuleAdd,
  specialRoomRuleUpdate
} from "@/api/systemSet/pmsSpecialRoomController";
...
export default {
  ...
  methods: {
    showDialog(rulePk) {
      ...
      if(rulePk) {
        //修改
        specialRoomRuleFind(rulePk).then(res=>{
    ...
    save() {
      ...
      if(this.rulePk) {
        //更新
        specialRoomRuleUpdate(data).then(res=>{
        ...
        //添加
        specialRoomRuleAdd(data).then(res=>{
          this.$message.success('保存成功')
          this.loading = false
          this.handleClose()
        }).finally(()=>{
