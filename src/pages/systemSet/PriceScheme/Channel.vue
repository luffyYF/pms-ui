<template>
  <div>
    <el-table
        :data="channelChildList.tableList"
        border
        :span-method="objectSpanMethod"
        v-loading="parentObj.loading"
        ...
        <el-table-column v-for="(key,value) in channelChildList.title" :key="value" :prop="value" :label="key">
            <template slot-scope="scope">
...
export default {
  ...
  created() {
    this.init();
    this.selectGrade();
    this.parentObj = this.childMsg;
  },
  watch: {
    checkedChannel(newValue, oldValue) {
      console.log(newValue)
      var tmp = [];
      
      for(var i = 0; i < this.contrast.length; i++) {
        for(var j = 0; j < newValue.length; j++) {
          if(this.contrast[i].channelPks.indexOf(newValue[j]) > -1) {
            console.log(this.contrast[i])
            tmp.push(this.contrast[i])
          }
        }
      }
      this.channelChildList.tableList = tmp;
  ...
  computed:{
    checkedChannel(){
      return this.childMsg.checkedChannel
    }
  },
  data() {
    return {
  ...
  methods: {
    init() {
      ...
      listPriceScheme({ priceFlag: "N" }).then(res => {
        if (res.code == 1) {
          this.channelChildList = res.data;
          this.contactArray = [];
          this.contactDot = 0;
          this.channelChildList.tableList.sort(
            this.sortBy("channelName", false)
          );
          this.contrast = this.channelChildList.tableList;
          for(var i = 0; i<this.contrast.length; i++) {
            ...
            if(i === 0){
              ...
              if(item.channelName === this.contrast[i-1].channelName) {
