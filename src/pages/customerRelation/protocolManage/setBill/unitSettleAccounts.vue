...
<div>
  <el-dialog class="setBillSettle" :title="title" top="100px" :visible.sync="dialogVisible" width="800px"
    ...
    <el-form ref="dataForm" size="mini" :rules="rules" :model="dataForm" label-width="110px">
        <el-row :gutter="24">
            <el-col :span="8">
                <el-form-item :label="this.type == 1?'单位名称':'中介名称'"  prop="contactName">
                    {{dataForm.contactName}}
                </el-form-item>
            </el-col>
            <el-col :span="10">
                <el-form-item label="账务选择"  prop="type">
                    <el-radio-group @change="typeChange()" v-model="dataForm.type" >
                        <el-radio :label="0">选择账务</el-radio>
                        <el-radio :label="1">按账套</el-radio>
                    </el-radio-group>
                    
                </el-form-item>
            </el-col>
            <el-col :span="6">
                <el-form-item label="" v-if="dataForm.type == 1" prop="type">
                    <el-button type="primary" size="mini" @click="changeSetBilLClick()">选择账套</el-button>
                </el-form-item>
             </el-col>
        </el-row>
    </el-form>
        ...
        <span style="margin-top:5px;" v-if="this.dataForm.type == 1">总计{{consumerBillList.length}}笔财务,共计{{consumerBillList | momeyFilter}}元</span>
        <span style="margin-top:5px;" v-if="this.dataForm.type == 0">总计{{currentChangeConsumerBillList.length}}笔财务,共计{{currentChangeConsumerBillList | momeyFilter}}元</span>
        ...
        <span style="margin-top:5px;" v-if="this.dataForm.type == 0">平衡数{{currentChangeConsumerBillList | momeyFilter2(settlementBillList)}}元</span>
        <span style="margin-top:5px;" v-if="this.dataForm.type == 1">平衡数{{consumerBillList | momeyFilter2(settlementBillList)}}元</span>
        <span slot="footer" class="dialog-footer">
            
            <el-button type="primary" size="mini" v-if="this.dataForm.type == 1" @click="saveBillClik()">保存</el-button>
    ...
    <el-dialog class="setBillSettle" title="账套选择" top="150px" :visible.sync="setBillDialog" width="700px" :close-on-click-modal="false" :before-close="handleSetBillClose">
        <el-table
            size="mini" 
            border 
            highlight-current-row
            :data="setBillList" 
            @row-dblclick="setBillDblClick"
            row-dblclick
            height="400px">
        ...
        <span slot="footer" class="dialog-footer">
            ...
            <el-button @click="setBillDialog = false" size="mini">取 消</el-button>
        </span>
    </el-dialog>
...
import {listBillByAgreement,listBillBySetBillPk,listSetBillByAgreementPk,listSettleBillByAgreement,unitSettleAccounts} from '@/api/bill'
  ...
  export default {
    data () {
      return {
        ...
        currentChangeConsumerBillList:[],//多选消费账单
    ...
    methods: {
        setBillDblClick(row, event){
            this.currentSetBill = row
            this.listBillBySetBillPk(this.currentSetBill.setBillPk,"UN_SET","CONSUMER")
            this.listBillBySetBillPk(this.currentSetBill.setBillPk,"UN_SET","SETTLEMENT")
            this.setBillDialog = false
        },
        changeSetBilLClick(){
            this.listSetBillByAgreementPk();
            this.setBillDialog = true
        },
        listSetBillByAgreementPk(){
            listSetBillByAgreementPk({agreementPk:this.dataForm.agreementPk,status:0}).then(res=>{
                this.setBillList = res.data
            })
        },
        typeChange(){
            this.consumerBillList = []
            this.settlementBillList = []
            if(this.dataForm.type == 0){
                this.listBillByAgreement(this.dataForm.agreementPk,'UN_SET')
                this.listSettleBillByAgreement()
            }

        },
        handleSelectionChange(val){
            this.currentChangeConsumerBillList = val
        },
        handleSetBillClose(){
            this.setBillDialog = false
        },
        ...
        saveBillClik(isSettle){
            if(this.dataForm.type == 1){
                    ...
                    if(isSettle && this.currentSetBill.settlementAmount != this.currentSetBill.consumptionAmount){
                        this.$message({ type: 'warning', message: '所选账务未平,不能结账' })
                        return
                    }
                    this.currentSetBill.createTime = null
                    this.currentSetBill.settleAccountTime = null
                    var data = {
                        setBillPo:this.currentSetBill,
                        billPos:list
                    }
                    if(isSettle){
                ...
                var momey2 = 0;
                var list2 = this.currentChangeConsumerBillList
                for(var i=0;i<list2.length;i++){
                    list2[i].createTime = null
                    momey2 = momey2 + parseFloat(list2[i].consumptionAmount)
                }
                if(momey2 != momey){
                    this.$message({ type: 'warning', message: '所选账务未平,不能结账' })
                    return
                ...
                var data = {
                    settleBillPos:list,
                    consumptionBillPos:list2
                }
                unitSettleAccounts(data).then(res => {
                    if(res.code == 1){
                        this.$message({ type: 'success', message: res.sub_msg })
                        this.dialogVisible = false
                        this.$emit('callback')
        ...
        listBillByAgreement(agreementPk,status){
            listBillByAgreement({agreementPk:agreementPk,status:status}).then(res=>{
                this.consumerBillList = res.data;
            })
        },
        listSettleBillByAgreement(){
            listSettleBillByAgreement({agreementPk:this.dataForm.agreementPk,status:"UN_SET"}).then(res=>{
                this.settlementBillList = res.data
            })
        },
        handleClose () {
