<template>
  <el-dialog title="编辑" width="40%" :visible.sync="dialogVisible">
    ...
    <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" :loading="loading" @click="saveUser()">确 定</el-button>
      </span>
  </el-dialog>
</template>
...
<script>
  ...
  import {getUserById, addUser, updateUser} from '@/api/upmsUser'
  ...
  export default {
    ...
    methods: {
      showDialog (id) {
        ...
        if (id === undefined) {
          this.form = new UpmsUser()
          // this.imageUrl = ''
        } else {
          getUserById(id).then(res => {
            this.form = new UpmsUser(res.data)
      ...
      saveUser () {
        this.$refs.form.validate(valid => {
          if (valid) {
            this.loading = true
            let service = this.form.userId === '' ? addUser(this.form) : updateUser(this.form)
            service.then(result => {
              let {msg, code} = result
              if (code !== 1) {
                this.$message({message: msg, type: 'error'})
              } else {
                bus.$emit('upmsUserChange')
