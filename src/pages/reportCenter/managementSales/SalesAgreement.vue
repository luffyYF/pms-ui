<template>
  <div>
    ...
    <el-col :span="24" id="print-salesAgreement">
      <div class="tabs">
        <div class="tavs-title">
          <h3 style="text-align:center">深圳市前海豪斯菲尔信息科技有限公司</h3>
          <h4 style="text-align:center">销售分析报表 - 协议单位</h4>
        </div>
        <div class="tabs-contetn">
          ...
          <table width="100%" border="1" style="border-collapse:collapse;border-color:black;font-family: 宋体;font-size: 14px;margin:0 auto;color:black;text-align: center;" cellpadding="6" cellspacing="0">
            ...
            <tr v-for="(item, index) in tableData" :key="index">
              ...
              <td><span v-if="tableData.length-1 != index">{{item | moneyFilter(sumObj,1)}}</span></td>
              <td>{{item.peopleNightRoomNum}}</td>
              <td><span v-if="tableData.length-1 != index">{{item | moneyFilter(sumObj,2)}}</span></td>
              <td>{{item.houseFeeIncome}}</td>
              <td><span v-if="tableData.length-1 != index">{{item | moneyFilter(sumObj,3)}}</span></td>
              <td>{{item.cateringFeeIncome}}</td>
              <td>{{item.orderFeeIncome}}</td>
              <td>{{item.consumptionAmount}}</td>
              <td><span v-if="tableData.length-1 != index">{{item | moneyFilter(sumObj,4)}}</span></td>
              <td><span v-if="tableData.length-1 != index">{{item | moneyFilter(sumObj,5)}}</span></td>
              <td><span v-if="tableData.length-1 !=index">{{item | moneyFilter(sumObj,6)}}</span></td>
              <td><span v-if="tableData.length-1 !=index">{{item | moneyFilter(sumObj,7)}}</span></td>
            </tr>
          </table>
  ...
  methods: {
    ...
    getListType(){
     ...
     print() {
      ...
      this.LODOP.ADD_PRINT_HTM(10,10,774,1103,document.getElementById("print-salesAgreement").innerHTML);
    }
  },
  filters:{
